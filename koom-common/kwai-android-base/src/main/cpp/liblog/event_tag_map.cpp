...
bool EventTagMap::emplaceUnique(uint32_t tag, const TagFmt &tagfmt, bool verbose) {
  ...
  static const char errorFormat[] =
      OUT_TAG ": duplicate tag entries %" PRIu32 ":%.*s:%.*s and %" PRIu32 ":%.*s:%.*s)\n";
    ...
    if (it != Idx2TagFmt.end()) {
      if (verbose) {
        fprintf(stderr, errorFormat, it->first, (int)it->second.first.length(),
                it->second.first.data(), (int)it->second.second.length(), it->second.second.data(),
                tag, (int)tagfmt.first.length(), tagfmt.first.data(), (int)tagfmt.second.length(),
                tagfmt.second.data());
      }
      ret = false;
    } else {
...
static const char *endOfTag(const char *cp) {
  while (*cp && (isalnum(*cp) || strchr("_.-@,", *cp)))
    ++cp;
  return cp;
}
  ...
  while (cp < endp) {
    if (*cp == '\n') {
      ...
      if (*cp == '#') {
        ...
        fprintf(stderr, OUT_TAG ": unexpected chars (0x%02x) in tag number on line %d\n", *cp,
                lineNum);
        errno = EINVAL;
        return -1;
      }
...
EventTagMap *android_openEventTagMap(const char *fileName) {
  ...
  for (which = 0; which < NUM_MAPS; ++which) {
    ...
    if (fileName)
      break; // Only allow one as specified
  }
  ...
  for (which = 0; which < NUM_MAPS; ++which) {
    if (fd[which] >= 0) {
      newTagMap->mapAddr[which] = mmap(NULL, end[which], which ? PROT_READ : PROT_READ | PROT_WRITE,
                                       which ? MAP_SHARED : MAP_PRIVATE, fd[which], 0);
      save_errno = errno;
      close(fd[which]); /* fd DONE */
      fd[which] = -1;
      if ((newTagMap->mapAddr[which] != MAP_FAILED) && (newTagMap->mapAddr[which] != NULL)) {

...
int android_log_processLogBuffer(struct logger_entry *buf, AndroidLogEntry *entry) {
  ...
  int i;
  if (buf->hdr_size < sizeof(logger_entry_v1)) {
...
static int android_log_printBinaryEvent(const unsigned char **pEventData, size_t *pEventDataLen,
  ...
  case EVENT_TYPE_INT:
    /* 32-bit signed int */
    {
      if (eventDataLen < sizeof(android_event_int_t))
        return -1;
  ...
  case EVENT_TYPE_FLOAT:
    /* float */
    {
      if (eventDataLen < sizeof(android_event_float_t))
        return -1;
  ...
  case EVENT_TYPE_STRING:
    ...
    {
      if (eventDataLen < sizeof(android_event_string_t))
        return -1;
      ...
      if (strLen < outBufLen) {
        ...
        if (!result)
          result = 1; /* if not truncated, return no room */
      }
      eventData += strLen;
      eventDataLen -= strLen;
      if (result != 0)
  ...
  case EVENT_TYPE_LIST:
    ...
    {
      if (eventDataLen < sizeof(android_event_list_t))
        return -1;
    ...
    if (findChar(&cp, &len, '|') && findChar(&cp, &len, INT_MAX)) {
      ...
      case TYPE_MONOTONIC: {
        ...
        if (val >= minute) {
          ...
          outCount = snprintf(outBuf, outBufLen, (val >= hour) ? "%02" PRIu64 ":" : "%" PRIu64 ":",
...
int android_log_processBinaryLogBuffer(
  ...
  if (buf->hdr_size >= sizeof(logger_entry_v3) && buf->lid == LOG_ID_SECURITY) {
    entry->priority = ANDROID_LOG_WARN;
  }
  if (buf->hdr_size >= sizeof(logger_entry_v4)) {
    entry->uid = buf->uid;
  }
  inCount = buf->len;
  if (inCount < sizeof(android_event_header_t))
    return -1;
...
char *android_log_formatLogLine(AndroidLogFormat *p_format, char *defaultBuffer,
  ...
  if (p_format->colored_output) {
    prefixLen = snprintf(prefixBuf, sizeof(prefixBuf), "\x1B[%dm", colorFromPri(entry->priority));
 ...
 */
  ...
  case FORMAT_PROCESS:
    ...
    len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen, "%c(%s%5d) ", priChar, uid,
                   entry->pid);
    break;
  case FORMAT_THREAD:
    len = snprintf(prefixBuf + prefixLen, sizeof(prefixBuf) - prefixLen, "%c(%s%5d:%5d) ", priChar,
                   uid, entry->pid, entry->tid);
    strcpy(suffixBuf + suffixLen, "\n");
    ++suffixLen;
    break;
  case FORMAT_RAW:

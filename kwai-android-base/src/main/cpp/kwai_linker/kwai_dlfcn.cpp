...
static int dl_iterate_callback_elf(dl_phdr_info *info, size_t size, void *data) {
  ALOGV("dl_iterate_callback_elf %s %p", info->dlpi_name, info->dlpi_addr);
  auto target = reinterpret_cast<DlFcn::dl_iterate_data *>(data);
  if (info->dlpi_addr != 0 && strstr(info->dlpi_name, target->info_.dlpi_name)) {
    target->info_.dlpi_addr = info->dlpi_addr;
    target->info_.dlpi_phdr = info->dlpi_phdr;
...
KWAI_EXPORT void *DlFcn::dlsym(void *handle, const char *name) {
  ALOGV("dlsym %s", name);
  KCHECKP(handle)
  if (android_api_ < __ANDROID_API_N__ || android_api_ >= __ANDROID_API_O__) {
  ...
  for (int i = 0; i < dlpi_phnum; ++i) {
    if (dlpi_phdr[i].p_type == PT_LOAD && dlpi_phdr[i].p_offset == 0) {
      ...
      ALOGV("PT_LOAD dlpi_addr %p p_vaddr %p elf_base_addr %p", dlpi_addr, dlpi_phdr[i].p_vaddr,
            elf_base_addr);
    }
      ...
      if (is_use_gnu_hash) {
        ...
        do {
          ...
          if (((gnu_chain_[n] ^ hash) >> 1) == 0 && strcmp(strtab + s->st_name, name) == 0) {
            ALOGV("find %s %p", name, dlpi_addr + s->st_value);
            return reinterpret_cast<void *>(dlpi_addr + s->st_value);
          }
        } while ((gnu_chain_[n++] & 1) == 0);
      } else {
        ALOGV("lookup use elf hash");
...
KWAI_EXPORT void *DlFcn::dlopen_elf(const char *lib_name, int flags) {
  ...
  if (!dl_iterate_phdr) {
    ...
    ALOGV("%s loaded in Android at %p", lib_path, elf_base_addr);
  } else {
    dl_iterate_data data{};
...
#define fatal(fmt, args...)                                                                        \
  ...
  for (k = 0; k < elf->e_shnum; k++, shoff += elf->e_shentsize) {
    ...
    ALOGV("%s: k=%d shdr=%p type=%x", __func__, k, sh, sh->sh_type);
  ...
  ALOGV("%s: ok, dynsym = %p, dynstr = %p", lib_path, ctx->dynsym, ctx->dynstr);
...
KWAI_EXPORT void *DlFcn::dlsym_size_elf(void *handle, const char *name, size_t *size) {
  KCHECKP(handle)
  int k;
  auto *ctx = (struct ctx *)handle;
  auto *dynsym = (ElfW(Sym) *)ctx->dynsym;
  ...
  for (k = 0; k < ctx->dynsym_num; k++, dynsym++)
    ...
    but a VMA for shared libs or exe files, so we have to subtract the bias */
      void *ret = ctx->load_addr + dynsym->st_value - ctx->bias;
      *size = dynsym->st_size;
      ALOGI("%s found at %p size %zu", name, ret, *size);
      return ret;
    }

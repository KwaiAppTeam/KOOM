...
int HprofStrip::ProcessHeap(const void *buf, int first_index, int max_len, int heap_serial_no,
  ...
  case HPROF_ROOT_VM_INTERNAL: {
    array_serial_no = ProcessHeap(buf, first_index + HEAP_TAG_BYTE_SIZE + OBJECT_ID_BYTE_SIZE,
  ...
  case HPROF_ROOT_JNI_GLOBAL: {
    ...
    array_serial_no = ProcessHeap(
        buf, first_index + HEAP_TAG_BYTE_SIZE + OBJECT_ID_BYTE_SIZE + JNI_GLOBAL_REF_ID_BYTE_SIZE,
        max_len, heap_serial_no, array_serial_no);
  } break;
  ...
  case HPROF_ROOT_THREAD_BLOCK: {
    array_serial_no = ProcessHeap(
        buf, first_index + HEAP_TAG_BYTE_SIZE + OBJECT_ID_BYTE_SIZE + THREAD_SERIAL_BYTE_SIZE,
        max_len, heap_serial_no, array_serial_no);
  } break;
  ...
  case HPROF_CLASS_DUMP: {
     ...
     */
    int constant_pool_index = first_index + HEAP_TAG_BYTE_SIZE /*tag*/
    ...
    int constant_pool_size = GetShortFromBytes((unsigned char *)buf, constant_pool_index);
    constant_pool_index += CONSTANT_POOL_LENGTH_BYTE_SIZE;
    for (int i = 0; i < constant_pool_size; ++i) {
      unsigned char type = ((
          unsigned char *)buf)[constant_pool_index + CONSTANT_POLL_INDEX_BYTE_SIZE /*pool index*/];
    ...
    for (int i = 0; i < static_fields_size; ++i) {
      unsigned char type = ((unsigned char *)buf)[static_fields_index + STRING_ID_BYTE_SIZE /*ID*/];
      static_fields_index += STRING_ID_BYTE_SIZE /*string ID*/ + BASIC_TYPE_BYTE_SIZE /*type*/
    ...
    int instance_fields_size = GetShortFromBytes((unsigned char *)buf, instance_fields_index);
    instance_fields_index += INSTANCE_FIELD_LENGTH_BYTE_SIZE;
    instance_fields_index += (BASIC_TYPE_BYTE_SIZE + STRING_ID_BYTE_SIZE) * instance_fields_size;

    array_serial_no =
        ProcessHeap(buf, instance_fields_index, max_len, heap_serial_no, array_serial_no);
  }
  ...
  case HPROF_INSTANCE_DUMP: {
    ...
    if (is_current_system_heap_) {
      ...
      strip_bytes_sum_ += instance_dump_index + U4 /*占位*/ + instance_size - first_index;
    }

    array_serial_no = ProcessHeap(buf, instance_dump_index + U4 /*占位*/ + instance_size, max_len,
  ...
  case HPROF_OBJECT_ARRAY_DUMP: {
    int length = GetIntFromBytes((unsigned char *)buf, first_index + HEAP_TAG_BYTE_SIZE +
                                                           OBJECT_ID_BYTE_SIZE +
  ...
  case HPROF_PRIMITIVE_ARRAY_DUMP: {
    ...
    int length = GetIntFromBytes((unsigned char *)buf, primitive_array_dump_index);
    ...
    primitive_array_dump_index += BASIC_TYPE_BYTE_SIZE /*value type*/ + value_size * length;
    ...
    array_serial_no =
        ProcessHeap(buf, primitive_array_dump_index, max_len, heap_serial_no, array_serial_no);
  } break;
  ...
  case HPROF_HEAP_DUMP_INFO: {
    ...
    if (is_current_system_heap_) {
      strip_index_list_pair_[strip_index_ * 2] = first_index;
      strip_index_list_pair_[strip_index_ * 2 + 1] = first_index + HEAP_TAG_BYTE_SIZE /*TAG*/
  ...
  case HPROF_PRIMITIVE_ARRAY_NODATA_DUMP: { // Obsolete.
    array_serial_no = ProcessHeap(buf, first_index + HEAP_TAG_BYTE_SIZE, max_len, heap_serial_no,
                                  array_serial_no);
  } break;
...
ssize_t HprofStrip::HookWriteInternal(int fd, const void *buf, size_t count) {
  ...
  if (tag == HPROF_TAG_HEAP_DUMP || tag == HPROF_TAG_HEAP_DUMP_SEGMENT) {
    ...
    ((unsigned char *)buf)[index + 3] = (unsigned char)((unsigned int)record_length & 0x000000ffu);
  }
  ...
  for (int i = 0; i < strip_index_; i++) {
    ...
    if (write_len > 0) {
      ...
      __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "HookWrite array i:%d writeLen<0:%lu", i,
                          write_len);
    }
...
HprofStrip::HprofStrip()
    : hprof_fd_(-1), strip_bytes_sum_(0), heap_serial_num_(0), hook_write_serial_num_(0),
      strip_index_(0), is_hook_success_(false), is_current_system_heap_(false) {
  std::fill(strip_index_list_pair_, strip_index_list_pair_ + arraysize(strip_index_list_pair_), 0);
}

ALWAYS_INLINE bool HprofStrip::IsHookSuccess() const { return is_hook_success_; }

void HprofStrip::SetHprofName(const char *hprof_name) { hprof_name_ = hprof_name; }
} // namespace leak_monitor
} // namespace kwai
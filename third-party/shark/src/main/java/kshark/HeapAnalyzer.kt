...
class HeapAnalyzer constructor(
    public val listener: OnAnalysisProgressListener
) {
  ...
  public fun FindLeakInput.analyzeGraph(
    ...
    val (applicationLeaks, libraryLeaks) = findLeaks(leakingObjectIds, false)
  ...
  public fun deduplicateShortestPaths(inputPathResults: List<ReferencePathNode>): List<ReferencePathNode> {
    SharkLog.d { "start deduplicateShortestPaths" }
    ...
    SharkLog.d { "end deduplicateShortestPaths" }
    return outputPathResults
  }

  public fun updateTrie(
  ...
  public fun FindLeakInput.computeRetainedSizes(pathFindingResults: PathFindingResults): List<Int>? {
    if (!computeRetainedHeapSize) {
      return null
    }

    SharkLog.d { "start computeRetainedSizes" }
  ...
  public fun FindLeakInput.buildLeakTraces(pathFindingResults: PathFindingResults): Pair<List<ApplicationLeak>, List<LibraryLeak>> {
    SharkLog.d { "start buildLeakTraces" }
    ...
    SharkLog.d { "end buildLeakTraces" }
    return applicationLeaks to libraryLeaks
  }

  public fun buildLeakTraceObjects(
  ...
  public fun buildReferencePath(
    ...
    return shortestChildPath.mapIndexed { index, childNode ->
      LeakTraceReference(
          ...
          declaredClassName = childNode.declaredClassName
      )
    }
  }

  public fun computeLeakStatuses(leakReporters: List<ObjectReporter>): List<Pair<LeakingStatus, String>> {
  ...
  public fun since(analysisStartNanoTime: Long): Long {
